...
- add a per-server agent which is installed automatically together with AWG
- ensure the agent is deployed to every managed server
- let the agent continue local traffic accounting when the panel is offline
- let the agent execute server-local panel tasks while the central panel is unavailable
- add deferred synchronization so the agent pushes local state and task results back to the panel after reconnect
...
- добавить per-server агент, который ставится автоматически вместе с AWG
- обеспечить установку агента на все managed-серверы
- дать агенту возможность продолжать локальный учёт трафика, если панель офлайн
- дать агенту возможность исполнять server-local задачи панели, пока центральная панель недоступна
- добавить отложенную синхронизацию, чтобы после восстановления связи агент отправлял в панель локальное состояние и результаты задач
