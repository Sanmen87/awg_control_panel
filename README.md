...
- current hybrid mode supports:
  - local-only mode without public panel URL
  - optional future web sync when `PANEL_PUBLIC_BASE_URL` is set
...
- agent can work without public panel exposure
- if panel web exposure is added later, the same agent can enable sync mode
...
- текущий гибридный режим поддерживает:
  - local-only режим без внешнего URL панели
  - будущий web-sync, если будет задан `PANEL_PUBLIC_BASE_URL`
...
- агент может работать без публичного web-доступа к панели
- если потом панель будет вынесена наружу, тот же агент сможет включить sync mode
