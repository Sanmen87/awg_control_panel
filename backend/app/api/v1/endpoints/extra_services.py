...
def _server_has_mtproxy(server: Server) -> bool:
    creds = ServerCredentialsService()
    command = wrap_with_optional_sudo(
        """
set -e
if command -v docker >/dev/null 2>&1; then
  if docker ps -a --format '{{.Names}}' | grep -E '^awg-mtproxy-[0-9]+$' >/dev/null 2>&1; then
    echo FOUND
    exit 0
  fi
fi
if ls -d /opt/awg-extra-services/mtproxy-* >/dev/null 2>&1; then
  echo FOUND
  exit 0
fi
echo MISSING
""".strip(),
    ...
    if result.exit_status != 0:
        raise RuntimeError(result.stderr.strip() or result.stdout.strip() or "Unable to inspect server for MTProxy")
...
def create_extra_service(
    ...
    if payload.service_type != "mtproxy":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Only MTProxy is supported right now")
    ...
    server, _, _ = target
    try:
        if _server_has_mtproxy(server):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="MTProxy is already present on this server. Import/adopt flow is required before a new install.",
            )
    except HTTPException:
        raise
    except Exception as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
    domain = _validate_mtproxy_script_domain(payload.domain)
    config = {
    ...
    item = ServiceInstance(
        ...
        public_endpoint=f"{server.host}:443",
    )
    try:
        db.add(item)
        db.commit()
...
def delete_extra_service(
    ...
    if server and item.service_type == "mtproxy":
        try:
...
def refresh_extra_service_status(
    ...
    if item.service_type != "mtproxy":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Only MTProxy refresh is supported right now")
    try:
...
def deliver_extra_service_email(
    ...
    if not item:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Extra service not found")
    if item.service_type != "mtproxy":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Only MTProxy email delivery is supported right now")
    server = db.query(Server).filter(Server.id == item.server_id).first()
    if not server:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Server not found")
    ...
    AuditService().log(
        ...
        resource_type="extra_service",
        resource_id=str(item.id),
        details=f"MTProxy access sent to {target_email}",
        user_id=current_user.id,
    )
